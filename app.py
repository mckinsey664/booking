...
#     # ---- Get URL params (selected entity & date)
#     entity_type = request.args.get("entity_type")   # "company" or "person"
#     entity_id = request.args.get("entity_id")
#     selected_date = request.args.get("date")

#     # ---- POST: User picked a time slot to reserve
#     if request.method == "POST":
...
#         # ✅ Check if this entity already has a meeting at that time
#         c.execute("""
#             SELECT id FROM reservations
#             WHERE entity_type=? AND entity_id=? AND date=? AND start_time=?
#         """, (entity_type, entity_id, selected_date, chosen_time))
#         if c.fetchone():
#             flash("❌ This entity already has a meeting at that time.", "danger")
...
#         c.execute("SELECT room_name FROM reservations WHERE date=? AND start_time=?", (selected_date, chosen_time))
...
#             company_name = c.execute("SELECT name FROM companies WHERE id=?", (entity_id,)).fetchone()["name"]
#             subject = f"Meeting Confirmation with {company_name}"
#             body = (
...
#                 f"🏠 Room: {free_room}\n\n"
#                 f"This meeting was booked by: {email}\n"
#             )
#         else:
#             person = c.execute(
#                 "SELECT first_name, last_name, email FROM approved_users WHERE id=?", (entity_id,)
#             ).fetchone()
#             if person:
...
#             subject = f"Meeting Confirmation with {full_name}"
#             body = (
#                 f"Hello,\n\nA meeting has been scheduled.\n\n"
#                 f"📅 Date: {selected_date}\n"
#                 f"⏰ Time: {chosen_time}\n"
...
#         #     VALUES (?,?,?,?,?,?,?)
#         # """, (
#         #     user["id"], entity_type, entity_id, selected_date,
#         #     chosen_time, free_room, ",".join(list(set(attendees)))
#         # ))
#         c.execute("""
#             INSERT INTO reservations (user_id, entity_type, entity_id, date, start_time, room_name, invites, status)
#             VALUES (?,?,?,?,?,?,?,?)
#         """, (
#             user["id"], entity_type, entity_id, selected_date,
#             chosen_time, free_room, ",".join(list(set(attendees))), "Pending"
#         ))
...
#         msg = Message(subject, sender=app.config['MAIL_USERNAME'], recipients=list(set(attendees)))
#         msg.body = body
#         mail.send(msg)

#         # flash(f"✅ Reservation confirmed! Your room is **{free_room}**.", "success")
#         flash(f"✅ Meeting request sent! Waiting for {company_name}'s approval.", "success")

#         return redirect(url_for("reserve", entity_type=entity_type, entity_id=entity_id, date=selected_date))

#     # ---- GET: Build Available Times
#     available_times = []
#     if entity_type and entity_id and selected_date:
#         if entity_type == "company":
#             # ✅ Pull slots from admin-defined company_slots table
#             c.execute("""
#                 SELECT start_time FROM company_slots
#                 WHERE company_id = ? AND date = ?
#                 ORDER BY start_time
#             """, (entity_id, selected_date))
#             slots = [row["start_time"] for row in c.fetchall()]

#             # Remove those already reserved
#             for t in slots:
#                 reserved = c.execute("""
#                     SELECT 1 FROM reservations
#                     WHERE entity_type='company' AND entity_id=? AND date=? AND start_time=?
#                 """, (entity_id, selected_date, t)).fetchone()
#                 if not reserved:
#                     available_times.append(t)
#         else:
#             # ✅ For people, default open times (10:00–15:30)
#             start_hour = 10
#             end_hour = 16
...
#     # ---- Display label for search field
#     display_label = request.args.get("search_label")
#     if not display_label and entity_type and entity_id:
#         if entity_type == "company":
...
#             if row: display_label = f"{row['first_name']} {row['last_name']}"
...
#     # ---- Get URL params
#     entity_type = "company"   # 🔒 Force company type
#     entity_id = request.args.get("entity_id")
#     selected_date = request.args.get("date")
...
#         # ✅ Check if this specific slot_id is already booked
#         c.execute("""
#             SELECT id FROM reservations
...
#             return redirect(url_for("reserve", entity_id=entity_id, date=selected_date))
...
#             chosen_time, free_room, email, "Pending", slot_id
#         ))
#         conn.commit()
...
#         c.execute("""
#             SELECT id, start_time, end_time FROM company_slots
#             WHERE company_id = ? AND date = ?
#             ORDER BY start_time
#         """, (entity_id, selected_date))
#         slots = c.fetchall()
...
#             """, (s["id"],)).fetchone()
#             if not taken:
#                 available_times.append(s)   # ✅ FIXED LINE

#     # ---- Display label
#     display_label = request.args.get("search_label")
#     if not display_label and entity_id:
#         row = c.execute("SELECT name FROM companies WHERE id=?", (entity_id,)).fetchone()
#         if row:
